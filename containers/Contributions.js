...
// services
import { ContributionsService, FundsTransfersService } from "../services";
...
@inject("store")
@observer
class ContributionsContainer extends React.Component {
  constructor(props) {
    super(props);
    ...
    this.service = new ContributionsService();
  ...
  loadData = async () => {
    ...
    const { status, data, error } = await this.service.list(
      store.contributions.page,
      store.contributions.size
    );
    store.ui.hideLoading();
    if (status !== 1) {
      alert(error);
    } else {
      if (store.contributions.total !== data.meta["resource-count"]) {
        store.contributions.setTotal(data.meta["resource-count"]);
      }
  ...
  handlePageChange = (evt, page) => {
    const { store } = this.props;
    store.contributions.setPage(page + 1);
    this.setState(() => ({ isLoaded: 0 }));
  };

  handleRowsPerPageChange = evt => {
    const { store } = this.props;
    store.contributions.setSize(evt.target.value);
    this.setState(() => ({ isLoaded: 0 }));
  };
        ...
        <Table className={classes.primetrustTable}>
          <TableHead>
            <TableRow>
              ...
              <TableCell>Funds Transfer</TableCell>
          ...
          <TableBody>
            {this.state.data.map((item, i) => (
              <TableRow key={i.toString()}>
                ...
                <TableCell>
                  {item["attributes"]["contributor-name"]}
                  <br />
                  {item["attributes"]["contributor-email"]}
                </TableCell>
                ...
                <TableCell>{item["attributes"]["reference"]}</TableCell>
                <TableCell
                  padding="checkbox"
                  className={classes.primetrustTableCellActions}
                >
          ...
          <TableFooter>
            <TableRow>
              <TablePagination
                page={store.contributions.page - 1}
                rowsPerPage={store.contributions.size}
                count={store.contributions.total}
...
export default withStyles(styles)(ContributionsContainer);
